...
productPaths.map(async product => {
  puppeteer.launch({ headless: true }).then(async browser => {
    ...
    for (const version of versions) {
      ...
      console.log(`Generating PDF for ${product.replace(/-/g, ' ')} ${version}`);
      ...
      await Promise.all(tocLinks.map(async (section, i) => {
        try {
          const page = await browser.newPage();
          await page.goto(path.join(section, 'index.html'));
          await page.emulateMedia('screen');
      ...
      console.log(`PDF compiled for ${product.replace(/-/g, ' ')} ${version}`);
    }
    await browser.close();
  });
});
