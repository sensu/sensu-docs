...
enable_round_robin |      |
-------------|------
description  | If `true`, enables [round robin scheduling][5] on PostgreSQL. Any existing round robin scheduling will be disrupted and will migrate to PostgreSQL as entities check in with their keepalives. The existing etcd scheduler state will gradually be deleted as the keepalives on the etcd scheduler keys expire over time. Otherwise, `false`.
required     | false
default      | false
type         | Boolean
