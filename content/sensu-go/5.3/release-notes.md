...
- Round robin check scheduling allows Sensu to execute checks for each entity within a subscription in turn, giving you the ability to rotate a check through a set of entities. To enable round-robin scheduling, set the `round_robin` check attribute to `true`. See the [check reference][18] for more information.
