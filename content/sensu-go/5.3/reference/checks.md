...
|round_robin |      |
-------------|------
description  | Round-robin check scheduling lets you distribute proxy check executions evenly over a group of Sensu agents. When set to `true`, Sensu executes the proxy check using each agent within the subscription in turn.
required     | false
type         | Boolean
