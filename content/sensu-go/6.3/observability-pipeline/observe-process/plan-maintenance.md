...
If you followed this guide to create the `check-http` check on the `sensu-site` entity, you might have noticed that the check does not include a [handler][14].
To observe the silenced entry's effect, add a handler to the `check-http` check definition (or recreate the [silenced entry][15] with your own entity and a check that includes a handler).
The handler must include the built-in [`not_silenced`][13] event filter.

{{% notice warning %}}
**WARNING**: By default, silenced events will be handled unless the handler includes the built-in [`not_silenced`](../../observe-filter/filters/#built-in-filter-not_silenced) event filter to discard silenced events.
{{% /notice %}}

Follow one of these guides to add a handler to your check:

- [Send data to Sumo Logic with Sensu][16]
- [Send email alerts with the Sensu Go Email Handler][17]
- [Send PagerDuty alerts with Sensu][18]
- [Send Slack alerts with handlers][19]
