...
{{% notice note %}}
**NOTE**: To enable read-only queries on a standby server, you must set `wal_level` to `hot_standby`.
If you will never connect to the server in standby mode, you can set `wal_level` to `archive` instead.
{{% /notice %}}
