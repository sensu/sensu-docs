...
If you’re deploying Sensu to your infrastructure, we recommend using one of our supported packages, Docker images, or [configuration management integrations][15].
...
Sensu backends requires persistent storage, a local cache, and three exposed ports:

- `3000` - Sensu web UI
- `8080` - Sensu API and connection to the backend via sensuctl
- `8081` - WebSocket API used by Sensu agents to connect to the backend

See the [hardware requirements][hardware] guide for deployment recommendations.
Sensu backends running in a clustered configuration require additional ports.
See the [guide to running a Sensu cluster][22].
<!-- Link to deployment guide -->

**Sensu agents** are lightweight clients that run on the infrastructure components you want to monitor.
Agents register automatically with Sensu as entities and are responsible for creating check and metrics events to send to the backend event pipeline.
...
Interact with and manage Sensu with the sensuctl command line utility.
...
To start using sensuctl, run `sensuctl configure` to log in with your user credentials, namespace, [Sensu backend][21] URL. To configure sensuctl using defaults:
...
For now, you should see a blank slate: no incidents or entities.
...
You should see a 200 OK response from the API and `"Healthy": true` if the backend is up and running.
...
To verify that the agent has automatically registered with Sensu and is sending keepalive events, open the entity page in the [Sensu web UI][ui] or run `sensuctl entity list`
