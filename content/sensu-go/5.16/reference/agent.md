...
Sensu `keepalives` are the heartbeat mechanism used to ensure that all registered agents are operational and can reach the [Sensu backend][2].
Sensu agents publish keepalive events that contain [entity][3] configuration data to the Sensu backend according to the interval specified by the [`keepalive-interval` flag][4].
...
| keepalive-timeout |      |
--------------------|------
description         | Number of seconds after a missing keepalive event until the Sensu backend considers the agent unresponsive.
type                | Integer
default             | `120`
example             | {{< highlight shell >}}# Command line example
