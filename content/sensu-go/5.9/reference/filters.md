...
`event.check.occurrences_watermark`  | integer | For incident and resolution events, the [number of preceding events][29] with an OK status (for incident events) or non-OK status (for resolution events)
