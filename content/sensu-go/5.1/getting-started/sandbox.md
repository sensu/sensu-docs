...
sensuctl entity list
        ID          Class    OS          Subscriptions                  Last Seen            
────────────────── ─────── ─────── ───────────────────────── ─────────────────────────────── 
sensu-go-sandbox   agent   linux   entity:sensu-go-sandbox   2019-01-19 00:13:03 +0000 UTC  
...
────────────────── ─────────── ──────── ──────── ────────── ─────────────────────────────── 
sensu-go-sandbox   keepalive                 0   false      2019-01-19 00:13:23 +0000 UTC  
{{< /highlight >}}
...
Now that we're generating Slack alerts, let's reduce the potential for alert fatigue by adding a filter that only sends Slack alerts when there's a change in the event state.

To accomplish this, we'll create a filter called `state_change_only` using sensuctl.


{{< highlight shell >}}
sensuctl filter create state_change_only --action allow --expressions "event.check.occurrences == 1"

sensuctl filter list
       Name          Action           Expressions           
─────────────────── ──────── ────────────────────────────── 
state_change_only   allow    event.check.occurrences == 1  
{{< /highlight >}}

Now we can assign the `state_change_only` filter to the keepalive handler so we'll only receive alerts when the sandbox keepalive event changes between OK, warning, and critical states.

{{< highlight shell >}}
sensuctl handler update keepalive
{{< /highlight >}}

When prompted for the filters selection, enter `state_change_only` to apply the state change filter.

{{< highlight shell >}}
? Filters: [? for help] state_change_only
{{< /highlight >}}
...
Use the `curl_timings-check.json` file provided with the sandbox to create a service check that runs `metrics-curl.rb` every 10 seconds on all entities with the `entity:sensu-go-sandbox` subscription and send it to the InfluxDB metrics handler pipeline:
...
In Sensu Go metrics are a core element of the data model, so we can build pipelines to handle metrics separately from the alerts!
