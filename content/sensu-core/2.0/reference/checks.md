...
-------------|------
description  | The metric format generated by the check command. Sensu supports the following metric formats: <br>`nagios_perfdata` ([Nagios Performance Data][nagios]) <br>`graphite_plaintext` ([Graphite Plaintext Protocol][graphite]) <br>`influxdb_line` ([InfluxDB Line Protocol][influx]) <br>`opentsdb_line` ([OpenTSDB Data Specification][open])<br><br>When a check includes an `output_metric_format`, Sensu will extract the metrics from the check output and add them to the [event data][24] within the `metrics` scope. For more information about extracting metrics using Sensu, see the [guide][23]. 
required     | false (true if `output_metric_handlers` is configured)
type         | String
example      | {{< highlight shell >}}"output_metric_format": "graphite_plaintext"{{< /highlight >}}

|output_metric_handlers    |      |
-------------|------
description  | The metric formats expected by the `handlers` specified in the check definition. Sensu supports the following metric handler formats: <br>`nagios_perfdata` ([Nagios Performance Data][nagios]) <br>`graphite_plaintext` ([Graphite Plaintext Protocol][graphite]) <br>`influxdb_line` ([InfluxDB Line Protocol][influx]) <br>`opentsdb_line` ([OpenTSDB Data Specification][open])<br><br>When a check includes one or more `output_metric_handlers`, Sensu will transform metrics from the format specified by the `output_metric_format` attribute into the formats specified by `output_metric_handlers` before adding the metrics to the event data. For more information about extracting metrics using Sensu, see the [guide][23]. 
required     | false
type         | Array
example      | {{< highlight shell >}}"output_metric_handlers": ["influx-db", "graphite_plaintext"]{{< /highlight >}}
