...
required     | false
type         | Array
example      | {{< code shell >}}"sentinels": [{"host": "10.0.1.33", "port": 26379}]{{< /code >}}
...
required     | true
type         | String
example      | {{< code shell >}}"host": "10.0.1.33"{{< /code >}}
...
required        | false
type            | String
example         | {{< code shell >}}"cert_chain_file": "/etc/sensu/ssl/redis_cert.pem"{{< /code >}}
...
required         | false
type             | String
example          | {{< code shell >}}"private_key_file": "/etc/sensu/ssl/redis_key.pem"{{< /code >}}
...
{{< code shell >}}
sudo wget -O /etc/redis/sentinel.conf http://docs.sensu.io/sensu-core/1.9/files/sentinel.conf{{< /code >}}
