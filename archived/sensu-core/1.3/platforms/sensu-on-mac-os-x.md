...
2. Install the package using the `installer` utility
   {{< code shell >}}
sudo installer -pkg sensu-1.3.0-1.pkg -target /{{< /code >}}
...
1. To configure the Sensu client service wrapper, copy the default service
   ...
   {{< code shell >}}
sudo cp /opt/sensu/embedded/Cellar/sensu/1.3.0/Library/LaunchDaemons/org.sensuapp.sensu-client.plist /Library/LaunchDaemons/org.sensuapp.sensu-client.plist{{< /code >}}
