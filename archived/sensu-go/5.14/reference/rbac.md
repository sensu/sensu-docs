...
In addition to built-in RBAC, Sensu includes [license-activated][33] support for authentication using external [authentication providers][32].
...
_**NOTE**: For licensed tier users,`sensuctl namespace list` will list only the namespaces that the current user has access to._
