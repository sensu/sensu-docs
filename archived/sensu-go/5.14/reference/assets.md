...
| annotations | |
-------------|------
description  | Non-identifying metadata that's meaningful to people and external tools interacting with Sensu.<br><br>In contrast to labels, annotations cannot be used in [API response filtering][api-filter], [sensuctl response filtering][sensuctl-filter], or [dashboard view filtering][50].
required     | false
